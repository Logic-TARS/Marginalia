...
桌面端采用固定顶栏与可滚动工作区。书库以自适应卡片网格组织；阅读器以 AI 面板、正文阅读区和笔记面板构成三栏；创作工作台以素材、感想、稿件三栏呈现连续流程。页面横向留白以 40px 为基准，面板间距主要为 16px，内部密度以 8px、12px、16px 递进。

在 1180px 以下，创作工作台从三栏降为两栏并让稿件区占满下一行；在 900px 以下，导航、页头、阅读面板与工作台全部转为单列，操作区允许换行；在 560px 以下，卡片内容和工具条进一步纵向堆叠。响应式变化优先保证文字完整和操作可达，不依赖横向压缩维持桌面结构。
